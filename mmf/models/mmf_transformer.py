...
class MMFTransformer(BaseTransformer):
    def __init__(self, config: BaseTransformerConfigType, *args, **kwargs):
        super().__init__(config)
        self.classification_labels = self.config.num_labels
        self.modality_keys: List = []
    ...
    def transformer_encoder(self, embedding, attention_mask):
    ...
    def postprocess_output(self, output: Tensor) -> Dict[str, Tensor]:
        ...
        """
        output_dict = {}
        output_dict["scores"] = output.contiguous().view(-1, self.classification_labels)
        return output_dict
